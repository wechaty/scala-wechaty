...
import TronNodeClient.ApiKeyClientInterceptor
import xcoin.blockchain.tron.services.TronApi.{TronNodeClientBuilder, TronNodeClientNetwork}
...
class TronNodeClient(protected val stub: ReactorWalletStub,
  ...
  extends TronApi
    with DefaultVoteSupport
    with DefaultContractSupport
    with TNCUSDTSupport
    with DefaultAccountSupport
    with TNCBlockSupport
    with DefaultResourceSupport
    with DefaultTransactionSupport {
